...
static char *r92su_debugfs_ ##name ## _read(struct ieee80211_hw *hw,	\
...
static ssize_t r92su_debugfs_hw_ioread_write(struct ieee80211_hw *hw,
					     const char *buf, size_t count)
{
	...
	rtlpriv->dbg.ring_tail %= R92SU_DEBUG_RING_SIZE;

	if (rtlpriv->dbg.ring_len < R92SU_DEBUG_RING_SIZE)
...
static char *r92su_debugfs_hw_ioread_read(struct ieee80211_hw *hw, char *buf,
					  size_t bufsize, ssize_t *ret)
{
	...
	while (rtlpriv->dbg.ring_len) {
		...
		rtlpriv->dbg.ring_head %= R92SU_DEBUG_RING_SIZE;
...
static ssize_t r92su_debugfs_hw_iowrite_write(struct ieee80211_hw *hw,
					      const char *buf, size_t count)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
