...
PTS_COMMON_INFO SearchAdmitTRStream(struct rtllib_device *ieee, u8*	Addr, u8 TID, TR_SELECT	TxRxSelect)
{
	...
	if (ieee->iw_mode == IW_MODE_APSTA) {
		if (ap_get_stainfo(ieee, Addr) == NULL) {
	...
	*ppTS = SearchAdmitTRStream(
	...
	else
	{
		...
		else
		{
			...
			DIRECTION_VALUE		Dir = 0;
			if ((ieee->iw_mode == IW_MODE_MASTER)
#ifdef ASL
				|| ((ieee->iw_mode == IW_MODE_APSTA) && (ap_get_stainfo(ieee,Addr)))
#endif
				) {
				Dir =((TxRxSelect==TX_DIR)?DIR_DOWN:DIR_UP);
			} else {
