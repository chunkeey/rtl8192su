			     ...
			     union iwreq_data *wrqu, char *b)
{
	...
	printk("==============>ieee->current_network.channel is %x\n",ieee->current_network.channel);
	ret = 0;
out:
	up(&ieee->wx_sem);
	return ret;
...
#endif	
	{
		...
		if (is_ap) {
			if (ieee->current_ap_network.channel == 0)
				return -1;
			fwrq->m = rtllib_wlan_frequencies[ieee->current_ap_network.channel-1] * 100000;
			fwrq->e = 1;	
		} else
#endif
		{
...
#ifdef ASL
		|| ((ieee->iw_mode == IW_MODE_APSTA) && (ieee->ap_state == RTLLIB_LINKED))
#endif
		)
		rtllib_start_send_beacons(ieee);
...
#ifdef ASL
	{
		if (ieee->iw_mode == IW_MODE_APSTA || ieee->iw_mode == IW_MODE_MASTER)
			wrqu->mode = IW_MODE_INFRA;
		else
