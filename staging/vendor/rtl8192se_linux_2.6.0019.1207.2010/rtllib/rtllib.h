...
#endif
#ifdef ASL
#ifndef IW_MODE_APSTA
#define IW_MODE_APSTA			8
#endif
#endif
...
#endif
#define		TCB_PS_FALG_REPLY_PS_POLL	BIT0	
#define		TCB_PS_FLAG_MCAST_DTIM		BIT1	
#define		TCB_PS_PRE_TRANSMIT			BIT2	
...
#define MAX_FW_SUPPORT_MACID_NUM			31	
...
#ifdef ASL
#define aAPSifsTime	 (((priv->rtllib->current_ap_network.mode == IEEE_A)||(priv->rtllib->current_ap_network.mode == IEEE_N_24G)||(priv->rtllib->current_ap_network.mode == IEEE_N_5G))? 16 : 10)
#endif
...
#define IsMgntQosData(pframe)		( (((u8)(pframe[0]) & 0xFC) == (RTLLIB_STYPE_QOS_DATA|RTLLIB_FTYPE_DATA) ) ? true : false)
#define IsMgntQosNull(pframe)		( (((u8)(pframe[0]) & 0xFC) == (RTLLIB_STYPE_QOS_NULL|RTLLIB_FTYPE_DATA) ) ? true : false)
#define IsMgntNull(pframe)		( (((u8)(pframe[0]) & 0xFC) == (RTLLIB_STYPE_NULLFUNC|RTLLIB_FTYPE_DATA) ) ? true : false)	
...
extern bool get_qos_queueID_maskAPSD(	u8 QueueID, u8 APSD);
...
#ifdef ASL
#define IEEE_AP_MAC_HASH_SIZE 31
struct ieee_ap_seq {
	u8 mac[ETH_ALEN];
	u16 seq_num[17];
	u16 frag_num[17];
...
static inline u8 Frame_QoS_EOSP(u8* buf)
{
	struct rtllib_hdr_3addr *hdr;
	u16 fc;
	hdr = (struct rtllib_hdr_3addr *)buf;
	fc = le16_to_cpu(hdr->frame_ctl);
	return (u8)((frameqos*)(buf + (((fc & RTLLIB_FCTL_TODS)&&(fc & RTLLIB_FCTL_FROMDS))? 30 : 24)))->field.eosp;
}
struct eapol {
	u8 snap[6];
	u16 ethertype;
...
typedef struct _SW_RATR_MASK{
	bool bused;
	u32	ratr_bitmap;
	u32	mask;
}SW_RATRMASK, *PSW_RATR_MASK;
...
struct sta_info {
	...
	struct sk_buff_head WmmPsQueue;	
	bool	FillTIM;
	u8	FillTIMCount;
	u32	LastActiveTime; 
	SW_RATRMASK swratrmask;
	u8 sec_state;
	u16			StaDataRate;  
	u32			StaSS; 
	u16			RetryFrameCnt;
...
struct rtllib_device {
	...
	u8 b4ac_Uapsd;
	u32 qos_capability;
	u32 current_qos_mode;
	bool bin_service_period;
#if defined(_RTL8192_EXT_PATCH_) || defined(ASL)
	u8 APExtChlOffset;
#endif
#ifdef ASL
	...
	int hostap;
      	...
      	enum rtllib_state ap_state;
	int ap_rate;       /* current rate */
	short ap_ssid_set;
	bool bbeacon_start;
	short ap_proto_started;
	short ap_proto_stoppping;
	PRT_HIGH_THROUGHPUT	pAPHTInfo;
	u8	ap_Regdot11HTOperationalRateSet[16];		
	u8	ap_Regdot11TxHTOperationalRateSet[16];		
	int apmode;       /* A, B, G */
	struct rtllib_network current_ap_network;
	SW_CAM_TABLE swapcamtable[TOTAL_CAM_ENTRY];
	struct sk_buff_head  skb_apaggQ[MAX_QUEUE_SIZE]; 
	u8 ap_amsdu_in_process;
	u16 ap_pairwise_key_type;
	u16 ap_group_key_type;
	SW_RATR_TABLE swapratrtable[8];
	u64 ap_assoc_sta_bitmap;
	int ap_tx_keyidx; /* default TX key index (crypt[tx_keyidx]) */	
	u8 rsn_ie[MAX_WPA_IE_LEN];
	int rsn_ie_len;
	int rsn_activated;
	int encrypt_activated;
	u8 bhidden_ssid;
	struct list_head ap_mac_hash[IEEE_AP_MAC_HASH_SIZE];
#ifdef CONFIG_WPS
	u8*	beacon_wps_ie;
	u16	beacon_wps_ie_len;
	u8*	probe_rsp_wps_ie;
	u16	probe_rsp_wps_ie_len;
#endif
...
#define RTLLIB_QUEUE_LIMIT 128
	...
	delayed_work_struct_rsl apdev_set_wep_crypt_wq;
#endif
#endif
...
#ifdef ASL
	void (*SetWirelessMode)(struct net_device* dev, u8 wireless_mode, bool is_ap);
#else
...
#ifdef _RTL8192_EXT_PATCH_
	void (*del_hwsec_cam_entry)(struct rtllib_device *ieee, u8 *addr, bool is_mesh, bool is_ap); 
...
extern u8 AMSDU_GetAggregatibleList(struct rtllib_device *ieee, struct sk_buff *pCurSkb, struct sk_buff_head	*pSendList, u8 queue_index, bool is_ap);
...
#ifdef ASL
extern u8 MgntQuery_MgntFrameTxRate(struct rtllib_device *ieee, bool is_ap);
#else
...
extern void rtllib_softmac_stop_protocol(struct rtllib_device *ieee, u8 mesh_flag, u8 ap_flag, u8 shutdown);
extern void rtllib_softmac_start_protocol(struct rtllib_device *ieee,u8 mesh_flag,u8 ap_flag);
...
extern void rtllib_stop_send_beacons(struct rtllib_device *ieee);
extern int rtllib_wpa_supplicant_ioctl(struct rtllib_device *ieee, struct iw_point *p, u8 is_mesh, u8 is_ap);

extern void notify_wx_assoc_event(struct rtllib_device *ieee);
extern void rtllib_ps_tx_ack(struct rtllib_device *ieee, short success);

#ifdef ASL
extern void softmac_mgmt_xmit(struct sk_buff *skb, struct rtllib_device *ieee, bool is_ap);
#else
...
#ifdef ASL
extern int rtllib_wx_get_freq(struct rtllib_device *ieee, struct iw_request_info *a, 
			     union iwreq_data *wrqu, char *b, bool is_ap);
#else
extern int rtllib_wx_get_freq(struct rtllib_device *ieee, struct iw_request_info *a,
			     union iwreq_data *wrqu, char *b);
#endif
...
#ifdef ASL
void HTSetConnectBwMode(struct rtllib_device* ieee, HT_CHANNEL_WIDTH	Bandwidth, HT_EXTCHNL_OFFSET	Offset, bool is_ap);
#else
...
#else
#ifdef ASL
extern void HTConstructCapabilityElement(struct rtllib_device* ieee, u8* posHTCap, u8* len, u8 IsEncrypt, u8 bIsBcn, bool bAssoc, bool bap);
#else
...
#ifdef ASL
extern u8 MgntQuery_TxRateExcludeCCKRates(struct rtllib_device *ieee, bool is_ap);
#else
extern u8 MgntQuery_TxRateExcludeCCKRates(struct rtllib_device *ieee);
#endif
#ifdef _RTL8192_EXT_PATCH_
...
#endif
#ifdef ASL
extern struct sk_buff* rtllib_null_func(struct rtllib_device *ieee,u8* daddr, bool bqos, u8 TID, bool beosp, short pwr, bool is_ap);
#endif
#ifdef ASL
extern void softmac_ps_mgmt_xmit(struct sk_buff *skb, struct rtllib_device *ieee, u8 queue_index, bool is_ap);
#else
extern void softmac_ps_mgmt_xmit(struct sk_buff *skb, struct rtllib_device *ieee, u8 queue_index);
#endif
...
#endif
extern int wme_downgrade_ac(struct sk_buff *skb);
extern struct rtllib_txb *rtllib_alloc_txb(int nr_frags, int txb_size, int gfp_mask);
