...
#ifdef ASL
/* rtllib_ap_rx_frame_mgtmt
 *
 * Responsible for handling management control frames
 *
 * Called by rtllib_rx */
static inline int
rtllib_ap_rx_frame_mgmt(struct rtllib_device *ieee, struct sk_buff *skb,
			struct rtllib_rx_stats *rx_stats, u16 type,
			u16 stype)
{
	/* On the struct stats definition there is written that
	 ...
	 */
	int hostapd_flag=1;
	struct rtllib_authentication* auth = NULL;
	struct rtllib_hdr_3addr * hdr = (struct rtllib_hdr_3addr *)skb->data;
	rx_stats->len = skb->len;
	switch (stype) {
		...
		case RTLLIB_STYPE_AUTH:
			auth = (struct rtllib_authentication*)skb->data;
			if (auth->transaction == 2 || auth->transaction == 4)
				hostapd_flag = 0;
			break;
		case RTLLIB_STYPE_DEAUTH:
		case RTLLIB_STYPE_DISASSOC:
			if(memcmp(hdr->addr3, ieee->current_network.bssid, ETH_ALEN) == 0) 
				hostapd_flag = 0;
			else
				hostapd_flag = 1;
		default:
	...
	if (ieee->hostap != 1)
		hostapd_flag = 0;
	if (hostapd_flag == 1) {
		printk("Sending management frame to hostapd: stype is %x\n",stype);
		if(stype == RTLLIB_STYPE_AUTH){
			dump_buf(skb->data,skb->len);
			printk("\n");
		}
#ifdef ASL_DEBUG_2
		printk("Sending management frame to hostapd");
#endif
		if ((memcmp(hdr->addr1, ieee->dev->dev_addr, ETH_ALEN))
		     ||((memcmp(hdr->addr3, ieee->current_network.bssid, ETH_ALEN)) 
		     && (memcmp(hdr->addr3, ieee->current_ap_network.bssid, ETH_ALEN))))
		{
			dev_kfree_skb_any(skb);
			return 0;
		}
		...
		if (skb2)
		{
			spin_lock_irqsave(&ieee->mgmt_rx_lock,flag);
			printk("===================>send hostapd\n");
			ap_send_to_hostapd(ieee,skb2);
			spin_unlock_irqrestore(&ieee->mgmt_rx_lock,flag);
		}
#endif
	} else {
#ifdef ASL_DEBUG_2
		printk("Handling in driver");
#endif
		return rtllib_rx_frame_mgmt(ieee, skb, rx_stats, type, stype);
	}
...
static unsigned char rtl_rfc1042_header[] =
{ 0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00 };
/* Bridge-Tunnel header (for EtherTypes ETH_P_AARP and ETH_P_IPX) */
static unsigned char rtl_bridge_tunnel_header[] =
...
static int is_duplicate_packet(struct rtllib_device *ieee,
				      struct rtllib_hdr_4addr *header, bool is_ap)
{
	...
	{
		struct list_head *p;
		struct ieee_ap_seq *entry = NULL;
		u8 *mac = header->addr2;
		int index = mac[5] % IEEE_AP_MAC_HASH_SIZE;
		list_for_each(p, &ieee->ap_mac_hash[index]) {
			entry = list_entry(p, struct ieee_ap_seq, list);
			if (!memcmp(entry->mac, mac, ETH_ALEN))
				break;
		}
		if (p == &ieee->ap_mac_hash[index]) {
			entry = kmalloc(sizeof(struct ieee_ap_seq), GFP_ATOMIC);
			if (!entry) {
				printk(KERN_WARNING "Cannot malloc new mac entry\n");
				return 0;
			}
			...
			entry->frag_num[tid] = frag;
			entry->packet_time[tid] = jiffies;
			list_add(&entry->list, &ieee->ap_mac_hash[index]);
			return 0;
		}
		last_seq = &entry->seq_num[tid];
		last_frag = &entry->frag_num[tid];
...
void rtllib_indicate_packets(struct rtllib_device *ieee, struct rtllib_rxb** prxbIndicateArray,u8  index)
{
	...
	for(j = 0; j<index; j++)
	{
		...
		/* convert hdr + possible LLC headers into Ethernet header */
			...
			if (sub_skb->len >= 8 &&
				((memcmp(sub_skb->data, rtl_rfc1042_header, SNAP_SIZE) == 0 &&
				  ethertype != ETH_P_AARP && ethertype != ETH_P_IPX) ||
				 memcmp(sub_skb->data, rtl_bridge_tunnel_header, SNAP_SIZE) == 0)) {
...
#ifdef ASL
				if (ap_get_stainfo(ieee, prxb->src))
					is_ap = true;
				else
					is_ap = false;
#endif
...
#ifdef ASL
				if (is_ap) {
					sub_skb->protocol = eth_type_trans(sub_skb, ieee->apdev);
					sub_skb->dev = ieee->apdev;
				} else
#endif
				{
		...
		bool			is_ap)
{
	...
	WinSize = pHTInfo->RxReorderWinSize;
...
int rtllib_rx_check_duplicate(struct rtllib_device *ieee, struct sk_buff *skb, u8 multicast)
{
...
#ifdef ASL
	else if (ieee->iw_mode == IW_MODE_APSTA) {
		if (type == RTLLIB_FTYPE_MGMT) {
			if (memcmp(hdr->addr3, ieee->current_network.bssid, ETH_ALEN) == 0)
				is_ap = false;
			else
				is_ap = true;
		} else if (type == RTLLIB_FTYPE_DATA) {
			rtllib_rx_extract_addr(ieee, hdr, dst, src, bssid);
			if (ap_get_stainfo(ieee, src))
				is_ap = true;
			else
				is_ap = false;
		} else {
			printk("=========>what type of packet???\n");
			dump_buf(skb->data,skb->len);
		}
	}
#endif
	...
	if( (ieee->pHTInfo->bCurRxReorderEnable == false) || 
		...
		multicast) {
		if (!multicast) {
			if (is_duplicate_packet(ieee, hdr, false)){
				return -1;
			}
		}
	}
...
#else
	if (!IsDataFrame(skb->data) || IsLegacyDataFrame(skb->data)) {
		if(!((type == RTLLIB_FTYPE_MGMT) && (stype == RTLLIB_STYPE_BEACON))){
			if (is_duplicate_packet(ieee, hdr, is_ap)){
				return -1;
			}
		}
	} else {
#ifdef ASL
		if ((ieee->iw_mode == IW_MODE_MASTER) ||
			((ieee->iw_mode == IW_MODE_APSTA) && is_ap)) {
		...
		if( (pHTInfo->bCurRxReorderEnable == false) || !net->qos_data.active ) {
			if (is_duplicate_packet(ieee, hdr, is_ap)){
				return -1;
	}
		} else {
		...
		if (GetTs(ieee, (PTS_COMMON_INFO*) &pRxTS, hdr->addr2, 
			...
			RTLLIB_DEBUG(RTLLIB_DL_ERR, "ERR!!%s(): No TS!! Skip the check!!\n",__FUNCTION__);
				printk("ERR!!%s(): No TS!! Skip the check!!\n",__FUNCTION__);
			return -1;
		}
	}
	}
...
int rtllib_rx_data_filter(struct rtllib_device *ieee, u16 fc, u8 *dst, u8 *src, u8 *bssid, u8 *addr2)
{
...
#ifdef ASL
	if ((ieee->iw_mode == IW_MODE_INFRA) || (ieee->iw_mode == IW_MODE_ADHOC)
		|| ((ieee->iw_mode == IW_MODE_APSTA) && (NULL == ap_get_stainfo(ieee, src)))) {
		net = &ieee->current_network;
	} else
		net = &ieee->current_ap_network;
#else
	...
	if (((fc & RTLLIB_FCTL_DSTODS) != RTLLIB_FCTL_DSTODS)
            && (compare_ether_addr(net->bssid, bssid) != 0) 
            && memcmp(net->bssid, zero_addr, ETH_ALEN)) {
		return -1;
	}
	...
	if ( ieee->IntelPromiscuousModeInfo.bPromiscuousOn  &&
		...
		if ((fc & RTLLIB_FCTL_TODS) && !(fc & RTLLIB_FCTL_FROMDS) &&
			(compare_ether_addr(dst, net->bssid) != 0) &&	   
			(compare_ether_addr(bssid, net->bssid) == 0)) {   
			return -1;
		}
	}
	...
	if(ieee->iw_mode == IW_MODE_MESH) {
		...
		if (is_multicast_ether_addr(dst) || is_broadcast_ether_addr(dst)) {
			if (memcmp(bssid, net->bssid, ETH_ALEN)) {
				return -1;
			}
		}
		...
		size_t hdrlen)
{
...
#endif
	{
		...
		if (ieee->iw_mode == IW_MODE_MASTER || ieee->iw_mode == IW_MODE_APSTA){
	    	 	if (ap_get_stainfo(ieee, hdr->addr2) == NULL) {
				if (ieee->iw_mode == IW_MODE_APSTA)
					*crypt = ieee->sta_crypt[idx];
				else 
					*crypt = NULL;
	    		} else {
			if(is_broadcast_ether_addr(hdr->addr1) ||
				is_multicast_ether_addr(hdr->addr1) || ieee->iswep) {
				 entry_sta = 0;
				 if(ieee->iswep == 0)
					idx=1;
			} else
				entry_sta = ap_cryptlist_find_station(ieee,hdr->addr2,1);/*the sender.Just find*/
			if (entry_sta <0)
				*crypt = NULL;
			else{
				*crypt = ieee->stations_crypt[entry_sta]->crypt[idx];
#ifdef ASL_DEBUG2
				if(!(*crypt))
					 printk("\n Find idx_sta,but crypt is NULL.iswep=%d,idx_sta=%d,idx=%d",ieee->iswep,entry_sta,idx);	
#endif
			}
			}
			if ((stype == RTLLIB_STYPE_AUTH) && (type == RTLLIB_FTYPE_MGMT)){
    				printk("===>entry_sta is %d,idx is %d\n",entry_sta,idx);
				printk("===>crypt is %p\n",crypt);
			}
		} else
#endif
		...
		size_t hdrlen)
{
	...
	{
#ifdef ASL_DEBUG_2
		printk("\nieee->host_decrypt : %d", ieee->host_decrypt);
		printk("\nfc & IEEE80211_FCTL_WEP : %x", (fc & IEEE80211_FCTL_WEP));
		printk("\nkeyidx : %d", keyidx);
                if (type == IEEE80211_FTYPE_DATA && keyidx == -5) {
                         printk("\n----------------------------------------\n");
                         for(i = 0; i < skb->len; i++)
		...
		bool is_ap)
{
	...
	for(i = 0; i<rxb->nr_subframes; i++) {
		...
		if (sub_skb) {
...
#ifdef ASL
			if ( is_ap && (ethertype != ETH_P_PAE)) {
				if((dst[0] == 0xff) && (dst[1] == 0xff) && (dst[2] == 0xff) && (dst[3] == 0xff) && (dst[4] == 0xff) && (dst[5] == 0xff)) {
#ifdef ASL_DEBUG
					printk("\nFrame is broadcasted!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
#endif
					skb2 = skb_clone(sub_skb, GFP_ATOMIC);
					if (skb2 == NULL)
						printk(KERN_DEBUG "%s: skb_clone failed for "
						       "frame\n", dev->name);
				} else if (memcmp(dst,dev->dev_addr, ETH_ALEN) != 0) {
#ifdef ASL_DEBUG
					printk("\nFrame is not for AP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
#endif
					skb2 = skb_clone(sub_skb, GFP_ATOMIC);
			...
			if (skb2 != NULL) {
#ifdef ASL_DEBUG
				printk("\nSending frame to wireless media!!!!");
#endif
		 ...
		 struct rtllib_rx_stats *rx_stats)
{
	...
	if (IsQoSDataFrame(skb->data) && (ieee->iw_mode == IW_MODE_INFRA)) {
		if (ieee->ps != RTLLIB_PS_DISABLED) {
			switch (Frame_QoSTID(skb->data)) {
				case 0:
				case 3:
					btrigger_AC=GET_BE_UAPSD(ieee->b4ac_Uapsd);
					break;
				case 1:
				case 2:
					btrigger_AC=GET_BK_UAPSD(ieee->b4ac_Uapsd);
					break;
				case 4:
				case 5:
					btrigger_AC=GET_VI_UAPSD(ieee->b4ac_Uapsd);
					break;
				case 6:
				case 7:
					btrigger_AC=GET_VO_UAPSD(ieee->b4ac_Uapsd);
					break;
			}

			if (btrigger_AC) {
				if ( Frame_QoS_EOSP(skb->data)) {
					printk("%s(): <<<<<<<<<< Leave APSD service period <<<<<<<<<<\n",__func__);
	...
	if ((ieee->iw_mode == IW_MODE_INFRA)  && (ieee->sta_sleep == LPS_IS_SLEEP) 
		&& (ieee->polling) && (!bToOtherSTA) && (!btrigger_AC)) {
	...
	if(ieee->pHTInfo->bCurRxReorderEnable == false ||pTS == NULL || bToOtherSTA){
		rtllib_rx_indicate_pkt_legacy(ieee, rx_stats, rxb, dst, src, false);
	}else{
#ifdef TCP_CSUM_OFFLOAD_RX
		rxb->tcp_csum_valid = rx_stats->tcp_csum_valid;
#endif		
		RxReorderIndicatePacket(ieee, rxb, pTS, SeqNum, false);
	}
		 ...
		 struct rtllib_rx_stats *rx_stats)
{
	...
	bool bToOtherSTA = false;
	int ret = 0, i = 0;
	int frame_authorized = 1;
	u8 bEnableRxReorder = false;
	int keyidx = 0;
	u16 ethertype;
	bool is_ap = false;
	bool is_eapol = false;
	/*If hostapd is not running,then do nothing.By lawrence*/
	if((ieee->iw_mode == IW_MODE_MASTER) && (ieee->hostap == 0)) {
		goto rx_dropped;
	}
	hdr = (struct rtllib_hdr_4addr *)skb->data;
	fc = le16_to_cpu(hdr->frame_ctl);
	...
	if ((type == RTLLIB_FTYPE_DATA) && (rtllib_is_eapol_frame(ieee, skb, hdrlen))) {
	}
	/* Get Mgnt crypt if encrypted */
	ret = apdev_rx_mgnt_get_crypt(ieee, skb, &crypt, hdrlen);
	if(ret == -1) {
		printk("====>rx_mgnt_get_crypt failed\n");
		goto rx_dropped;
	}
	/* Filter CTRL Frame */
	if (type == RTLLIB_FTYPE_CTL) {
		if( (stype == RTLLIB_STYPE_PSPOLL) && (memcmp(dst, dev->dev_addr, ETH_ALEN))){
				u8 saddr[ETH_ALEN] = {0};
				memcpy(saddr,skb->data+10, ETH_ALEN);
				psta = ap_get_stainfo(ieee, saddr);
				if(NULL != psta){
					ap_ps_on_pspoll(ieee, fc, saddr, psta);
				}
#ifdef NOT_YET				
				else{
	...
	if (type == RTLLIB_FTYPE_MGMT) {
		...
		if(crypt) {
			if (strcmp(crypt->ops->name, "WEP") == 0) {
			    	if(stype == RTLLIB_STYPE_AUTH){
				    	printk("dump auth data\n");
					dump_buf(skb->data,skb->len);
					printk("\n");
				}
				if (ieee->host_decrypt && (fc & RTLLIB_FCTL_WEP) && (stype == RTLLIB_STYPE_AUTH))
				    ieee->need_sw_enc = 1;
				if (ieee->host_decrypt && (fc & RTLLIB_FCTL_WEP) && (stype == RTLLIB_STYPE_AUTH) &&
				    (keyidx = rtllib_rx_frame_decrypt(ieee, skb, crypt)) < 0)
				{
		...
		if (rtllib_ap_rx_frame_mgmt(ieee, skb, rx_stats, type, stype))
			goto rx_dropped;
		else
			goto rx_exit;
	}
	...
	psta = ap_get_stainfo(ieee, src);
	if (psta != NULL) {
	if (IsQoSDataFrame(skb->data))
		TID = Frame_QoSTID(skb->data);
	else
	   	TID = 0;
	ap_ps_update_station_psstate(ieee, src, fc, TID);
	if (IsMgntQosNull(skb->data) || IsMgntNull(skb->data)){
		    	printk("====================>IsMgntQosNull %d,IsMgntNull %d\n",IsMgntQosNull(skb->data),IsMgntNull(skb->data));
		goto rx_dropped;
	}
	
	frame_authorized = 0;
	switch(ap_is_data_frame_authorized(ieee, dev, src, dst)) {
		case 1:
#ifdef ASL_DEBUG
			printk("\nData frame authorized!");
#endif
			frame_authorized = 1;
			break;
		case -1:
#ifdef ASL_DEBUG
			printk("\nData frame not authorized - from unassociated client!");
#endif
			break;
		case -2:
			frame_authorized = 1;
#ifdef ASL_DEBUG
			printk("\nData frame not authorized - to a bridge!");
#endif
			break;
		case -3:
#ifdef ASL_DEBUG
			printk("\nData frame not authorized!");
#endif
			break;
		default:
			break;
	...
	if (ieee->iw_mode == IW_MODE_MASTER)
		is_ap = true;
	ethertype = (skb->data[6] << 8) | skb->data[7];
	if (ethertype != ETH_P_PAE) {
		if(frame_authorized) {
#ifdef ASL_DEBUG
			printk("\nFrame is authorized!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
	...
	if(skb->len == hdrlen){
		printk("====>skb->len is too small dropped\n");
		goto rx_dropped;
	}
	if(NULL != psta)
		psta->LastActiveTime = jiffies;
	...
	ret = rtllib_rx_decrypt(ieee, &skb, rx_stats, crypt, hdrlen);
	if(ret == -1){ 
		printk("====>rx_decrypt dropped\n");
		goto rx_dropped;
	}
	else if(ret == -2){
		printk("====>ex_decrypt exit\n");
		goto rx_exit;
		}
	...
	hdr = (struct rtllib_hdr_4addr *) skb->data;
#ifdef ASL
	if(((ieee->current_network.qos_data.active && (NULL == psta)) || (ieee->current_ap_network.qos_data.active && psta)) 
#else
	if((ieee->current_network.qos_data.active) 
#endif
		&& IsQoSDataFrame(skb->data) 
		&& !is_multicast_ether_addr(hdr->addr1) && !is_broadcast_ether_addr(hdr->addr1) 
		&& (!bToOtherSTA))
	{
		TID = Frame_QoSTID(skb->data);
	...
	if(rtllib_is_eapol_frame(ieee, skb, hdrlen)) {
        ...
        { 
		ap_update_stainfo(ieee,hdr,rx_stats);
        }
#endif
	...
	if(parse_subframe(ieee,skb,rx_stats,rxb,src,dst) == 0) {
		...
		kfree(rxb);
		printk("====>parse_subframe failed, dropped\n");
		rxb = NULL;
		goto rx_dropped;
	}
	/* Update WAPI PN */
...
#endif
	/* Indicate packets to upper layer or Rx Reorder */
	if(psta->htinfo.bCurRxReorderEnable)
			bEnableRxReorder = true;
	
	if((psta && (ieee->pAPHTInfo->bCurRxReorderEnable == false || psta->htinfo.bCurRxReorderEnable == false 	
		||pTS == NULL || bToOtherSTA)) 
		|| (!psta && (ieee->pHTInfo->bCurRxReorderEnable == false 
		||pTS == NULL || bToOtherSTA)) || is_eapol){
		rtllib_rx_indicate_pkt_legacy(ieee, rx_stats, rxb, dst, src, is_ap);
	}else{
#ifdef TCP_CSUM_OFFLOAD_RX
		rxb->tcp_csum_valid = rx_stats->tcp_csum_valid;
#endif		
		RxReorderIndicatePacket(ieee, rxb, pTS, SeqNum, is_ap);
	}
		 ...
		 struct rtllib_rx_stats *rx_stats)
{
	...
	if(ieee->softmac_process_bt_pkt)
		bt_data = ieee->softmac_process_bt_pkt(skb, ieee->dev);
                                             ...
                                             struct rtllib_network *network)
{
	...
	struct rtllib_qos_parameter_info param_element;
		...
		network->qos_data.qos_mode |= QOS_WMM;
		if (network->flags & NETWORK_HAS_QOS_PARAMETERS) {
			if(param_element.info_element.ac_info & 0x80 ) 
				network->qos_data.qos_mode |= QOS_WMM_UAPSD;
			else
				network->qos_data.qos_mode &=~ QOS_WMM_UAPSD;
		}
        }
