...
#else	
#ifdef ASL
void HTConstructCapabilityElement(struct rtllib_device* ieee, u8* posHTCap, u8* len, u8 IsEncrypt, u8 bIsBcn, bool bAssoc, bool bap)
#else
void HTConstructCapabilityElement(struct rtllib_device* ieee, u8* posHTCap, u8* len, u8 IsEncrypt, bool bAssoc)
#endif	
#endif	
{	
	PRT_HIGH_THROUGHPUT	pHT = NULL;
...
#ifdef ASL
	if (bap)
		memcpy(pCapELE->MCS, ieee->ap_Regdot11HTOperationalRateSet, 16);
	else
#endif
...
#else
#ifdef ASL
	if ( (ieee->iw_mode == IW_MODE_ADHOC) || (ieee->iw_mode == IW_MODE_MASTER) || (ieee->iw_mode == IW_MODE_APSTA)) 
#else
	if ( (ieee->iw_mode == IW_MODE_ADHOC) || (ieee->iw_mode == IW_MODE_MASTER)) 
#endif
#endif
	{
		pHTInfoEle->ControlChl 			= net->channel; 
#ifdef _RTL8192_EXT_PATCH_
...
#else
#ifdef ASL
		if (ieee->iw_mode == IW_MODE_APSTA && ieee->state == RTLLIB_LINKED)
			pHTInfoEle->ExtChlOffset 			= ((pHT->bRegBW40MHz == false)?HT_EXTCHNL_OFFSET_NO_EXT:
											ieee->APExtChlOffset);
#endif
			pHTInfoEle->ExtChlOffset 			= ((pHT->bRegBW40MHz == false)?HT_EXTCHNL_OFFSET_NO_EXT:
...
void HTOnAssocRsp(struct rtllib_device *ieee)
{
	PRT_HIGH_THROUGHPUT	pHTInfo = ieee->pHTInfo;
#ifdef ASL
	PRT_HIGH_THROUGHPUT	pAPHTInfo = ieee->pAPHTInfo;
#endif
...
#ifdef ASL
	HTSetConnectBwMode(ieee, (HT_CHANNEL_WIDTH)(pPeerHTCap->ChlWidth), (HT_EXTCHNL_OFFSET)(pPeerHTInfo->ExtChlOffset), false);
#else
	HTSetConnectBwMode(ieee, (HT_CHANNEL_WIDTH)(pPeerHTCap->ChlWidth), (HT_EXTCHNL_OFFSET)(pPeerHTInfo->ExtChlOffset));
#endif
	printk("pHTInfo->bCurBW40MHz is %d\n",pHTInfo->bCurBW40MHz);
	...
	if (ieee->iw_mode == IW_MODE_APSTA) {
		if(ieee->rtllib_ap_sec_type && (ieee->rtllib_ap_sec_type(ieee)&(SEC_ALG_CCMP))){
			pHTInfo->CurrentAMPDUFactor = pHTInfo->AMPDU_Factor = 0;  
			ieee->pAPHTInfo->CurrentAMPDUFactor = ieee->pAPHTInfo->AMPDU_Factor = 0;
			printk("%s: In MSTA mode, AP is encrypted with AES, force CurrentAMPDUFactor to 8K!\n", __func__);
		}
	}
#endif
	...
	if (ieee->iw_mode == IW_MODE_APSTA) {
		if(ieee->rtllib_ap_sec_type && (ieee->rtllib_ap_sec_type(ieee)&(SEC_ALG_CCMP))){
			pHTInfo->bCurRxReorderEnable = pHTInfo->bRegRxReorderEnable = 0;
			ieee->pAPHTInfo->bCurRxReorderEnable = ieee->pAPHTInfo->bRegRxReorderEnable = 0;
			printk("%s: In MSTA mode, AP is encrypted with AES, force closing RxReorder!\n", __func__);
		}
	}
#endif
...
#ifdef ASL
void HTSetConnectBwModeCallback(struct rtllib_device* ieee, bool is_ap);
#else
...
#endif
{
	...
	if(Bandwidth==HT_CHANNEL_WIDTH_20_40)
	 {
		if(net->channel<2 && Offset==HT_EXTCHNL_OFFSET_LOWER)
...
#ifdef ASL
	HTSetConnectBwModeCallback(ieee, is_ap);
#else
...
#endif
{
	...
	if(pHTInfo->bCurBW40MHz)
	{
		if(pHTInfo->CurSTAExtChnlOffset==HT_EXTCHNL_OFFSET_UPPER)
			ieee->set_chan(ieee->dev, net->channel+2);
