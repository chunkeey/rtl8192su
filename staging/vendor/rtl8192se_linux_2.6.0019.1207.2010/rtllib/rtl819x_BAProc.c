...
#else	
#ifdef ASL
	if ((ieee->iw_mode == IW_MODE_MASTER) ||(ieee->iw_mode == IW_MODE_APSTA && (ap_get_stainfo(ieee, Dst))))
		memcpy(BAReq->addr3, ieee->current_ap_network.bssid, ETH_ALEN);
...
#ifdef ASL
	if ((ieee->iw_mode == IW_MODE_MASTER) ||(ieee->iw_mode == IW_MODE_APSTA && (ap_get_stainfo(ieee, dst))))
		memcpy(Delba->addr3, ieee->current_ap_network.bssid, ETH_ALEN);
	else if (ieee->state == RTLLIB_LINKED)
#endif
	memcpy(Delba->addr3, ieee->current_network.bssid, ETH_ALEN);
#endif
...
void rtllib_send_ADDBAReq(struct rtllib_device* ieee, u8*	dst, PBA_RECORD	pBA)
{
	...
	if (skb)
	{
		...
		if (ieee->iw_mode == IW_MODE_MASTER) {
			if (ap_get_stainfo(ieee, dst) != NULL)
				softmac_mgmt_xmit(skb, ieee, true);
			else {
				RTLLIB_DEBUG(RTLLIB_DL_ERR, "This station is not connected yet %s()\n", __FUNCTION__);
				return;
			}
...
#ifdef ASL
	if ((ieee->iw_mode == IW_MODE_MASTER) || 
		((ieee->iw_mode == IW_MODE_APSTA) && (ap_get_stainfo(ieee, dst)))) {
